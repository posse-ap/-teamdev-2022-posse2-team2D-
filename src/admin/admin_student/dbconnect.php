...
if (isset($_GET['search_name']) && strlen($_GET['search_company']) == 0 && strlen($search_date) == 0) :
  $search_n = $_GET['search_name'];
  $apply_info_stmt = $db->prepare("SELECT * FROM apply_info WHERE name = '$search_n' order by created_at desc");
  $apply_info_stmt->execute();
  $apply_infos = $apply_info_stmt->fetchAll();
elseif (isset($_GET['search_company']) && strlen($_GET['search_name']) == 0 && strlen($search_date) == 0) :
  $search_c = $_GET['search_company'];
  $apply_info_stmt = $db->prepare("SELECT distinct apply_info.* FROM agent_user inner JOIN apply_info ON agent_user.user_id=apply_info.id inner JOIN agent ON agent_user.agent_id=agent.id WHERE agent_name = '$search_c' order by created_at desc");
  $apply_info_stmt->execute();
  $apply_infos = $apply_info_stmt->fetchAll();
elseif (isset($_GET['search_name']) && isset($_GET['search_company']) && strlen($search_date) == 0) :
  if (strlen($_GET['search_name']) == 0 or strlen($_GET['search_company']) == 0) :
    ...
    $search_n = $_GET['search_name'];
    $search_c = $_GET['search_company'];
    $apply_info_stmt = $db->prepare("SELECT distinct apply_info.* FROM agent_user inner JOIN apply_info ON agent_user.user_id=apply_info.id inner JOIN agent ON agent_user.agent_id=agent.id WHERE name = ? and agent_name = ?order by created_at desc");
    $apply_info_stmt->bindValue(1, $search_n, PDO::PARAM_STR);
    $apply_info_stmt->bindValue(2, $search_c, PDO::PARAM_STR);
    $apply_info_stmt->execute();
    $apply_infos = $apply_info_stmt->fetchAll();
...
elseif (isset($_GET['search_name']) && isset($search_date) && strlen($_GET['search_company']) == 0) :
  if (strlen($_GET['search_name']) == 0 or strlen($search_date) == 0) :
    ...
    $search_n = $_GET['search_name'];
    $apply_info_stmt = $db->prepare("SELECT distinct apply_info.* FROM agent_user inner JOIN apply_info ON agent_user.user_id=apply_info.id inner JOIN agent ON agent_user.agent_id=agent.id WHERE name = ? and created_at like ? order by created_at desc");
    $apply_info_stmt->bindValue(1, $search_n, PDO::PARAM_STR);
...
elseif (isset($_GET['search_company']) && isset($search_date) && strlen($_GET['search_name']) == 0) :
  if (strlen($_GET['search_company']) == 0 or strlen($search_date) == 0) :
    $apply_info_stmt = $db->prepare("SELECT distinct apply_info.* FROM agent_user inner JOIN apply_info ON agent_user.user_id=apply_info.id inner JOIN agent ON agent_user.agent_id=agent.id order by created_at desc");
    $apply_info_stmt->execute();
    $apply_infos = $apply_info_stmt->fetchAll();
  else :
    $search_c = $_GET['search_company'];
    $apply_info_stmt = $db->prepare("SELECT distinct apply_info.* FROM agent_user inner JOIN apply_info ON agent_user.user_id=apply_info.id inner JOIN agent ON agent_user.agent_id=agent.id WHERE agent_name = ? and created_at like ? order by created_at desc");
    $apply_info_stmt->bindValue(1, $search_c, PDO::PARAM_STR);
    $apply_info_stmt->bindValue(2, $like, PDO::PARAM_STR);
    $apply_info_stmt->execute();
    $apply_infos = $apply_info_stmt->fetchAll();
  endif;
elseif (isset($_GET['search_name']) && isset($_GET['search_company']) && isset($search_date)) :
  $search_n = $_GET['search_name'];
  $search_c = $_GET['search_company'];
  $apply_info_stmt = $db->prepare("SELECT distinct apply_info.* FROM agent_user inner JOIN apply_info ON agent_user.user_id=apply_info.id inner JOIN agent ON agent_user.agent_id=agent.id WHERE name = ? and agent_name = ? and created_at like ? order by created_at desc");
  $apply_info_stmt->bindValue(1, $search_n, PDO::PARAM_STR);
  $apply_info_stmt->bindValue(2, $search_c, PDO::PARAM_STR);
...
elseif (isset($_GET['search_name']) && isset($search_date) && strlen($_GET['search_company']) == 0) :
  if (strlen($_GET['search_name']) == 0 or strlen($search_date) == 0) :
    ...
    $search_n = $_GET['search_name'];
    $info_num_stmt = $db->prepare("SELECT COUNT(*) FROM agent_user inner JOIN apply_info ON agent_user.user_id=apply_info.id inner JOIN agent ON agent_user.agent_id=agent.id WHERE name = ? and created_at like ?");
    $info_num_stmt->bindValue(1, $search_n, PDO::PARAM_STR);
...
elseif (isset($_GET['search_company']) && isset($search_date) && strlen($_GET['search_name']) == 0) :
  if (strlen($_GET['search_company']) == 0 or strlen($search_date) == 0) :
    ...
    $search_c = $_GET['search_company'];
    $info_num_stmt = $db->prepare("SELECT COUNT(*) FROM agent_user inner JOIN apply_info ON agent_user.user_id=apply_info.id inner JOIN agent ON agent_user.agent_id=agent.id WHERE agent_name = ? and created_at like ?");
    $info_num_stmt->bindValue(1, $search_c, PDO::PARAM_STR);
    $info_num_stmt->bindValue(2, $like, PDO::PARAM_STR);
    $info_num_stmt->execute();
    $info_nums = $info_num_stmt->fetchAll();
  endif;
elseif (isset($_GET['search_name']) && isset($_GET['search_company']) && isset($search_date)) :
  $search_n = $_GET['search_name'];
  $search_c = $_GET['search_company'];
  $info_num_stmt = $db->prepare("SELECT COUNT(*) FROM agent_user inner JOIN apply_info ON agent_user.user_id=apply_info.id inner JOIN agent ON agent_user.agent_id=agent.id WHERE name = ? and agent_name = ? and created_at like ?");
  $info_num_stmt->bindValue(1, $search_n, PDO::PARAM_STR);
  $info_num_stmt->bindValue(2, $search_c, PDO::PARAM_STR);
