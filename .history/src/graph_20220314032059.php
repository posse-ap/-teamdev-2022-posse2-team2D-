...
$data_time->execute();
$data_time_list = $data_time->fetch();

// 棒グラフ（日ごとに時間を足してる）
$graphTimeStmt = $db->query("SELECT study_date, sum(study_time) as sum_study_time from study_data WHERE DATE_FORMAT(study_date, '%Y%m')=DATE_FORMAT(NOW(), '%Y%m') group by study_date");
...
<script>
    ...
    // ]);
    // }
    <?php foreach ($graphTime as $graph) { ?>
